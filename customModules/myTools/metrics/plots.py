...
def rate_plot(y_true, y_pred, weights, bin_size=0.02, combine_nu_tau_cc=False, savepath=None):
    ...
    x_offset = 0.03
    if counts.shape[0] == 2 or (counts.shape[0] == 3 and combine_nu_tau_cc):
        y_offset = 0
...
def cut_survival_plot(y_true, y_pred, weights, ids, combine_nu_tau_cc=False, savepath=None):
    # Sort by id
    sorted_ind = np.argsort(ids)
    # Find unique ids and their indices
    u_val, u_ind = np.unique(ids[sorted_ind], return_index=True)
    ...
    # Sum weights for each bin
    # Find a more pythonic way of doing this
    for i in range(len(inds)):
        counts[label_mask[i, :], inds[i]] += weights[i]
    ...
    for i in range(counts.shape[0]):
        if combine_nu_tau_cc:
            ...
            # This makes label 2 redundant, so we can skip it
                ...
                continue
        ax.plot(bins, counts[i], label=labels[str(i)])
