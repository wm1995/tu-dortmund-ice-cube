...
def main(
    ...
    """
    Retrains layers for a given Keras model
    ...
    Arguments:
        ...
        no_frozen - the number of layers to be frozen (top layers frozen first)
    ...
    # Freeze appropriate number of layers
    for layer in model.layers[:no_frozen]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
            '-f', '--no-frozen', 
            help='sets the number of layers to be frozen (layers frozen from top down)',
            type=int, dest='no_frozen',
            required=True
        )

    parser.add_argument(
            '-e', '--no-epochs', 
            help='sets the number of epochs (default: 1e3)',
